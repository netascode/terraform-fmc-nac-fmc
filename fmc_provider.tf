...
provider "fmc" {
  fmc_username             = try(local.envs["FMC_USERNAME"], var.FMC_USERNAME)
  fmc_password             = try(local.envs["FMC_PASSWORD"], var.FMC_PASSWORD)
  fmc_host                 = try(local.envs["FMC_HOSTNAME"], var.FMC_HOSTNAME)
