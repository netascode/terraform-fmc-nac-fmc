...
locals {
  ...
  # External objects
  # Add only if not managed as internal objects in case module instance includes YAML generated by itself.
  map_hosts_external = {
    for key, value in try(local.data.hosts, {}) : key => value if !contains(keys(local.map_hosts_internal), key)
  }
...
locals {
  ...
  map_networks_external = {
    for key, value in try(local.data.networks, {}) : key => value if !contains(keys(local.map_networks_internal), key)
  }
...
locals {
  ...
  # External objects
  map_ranges_external = {
    for key, value in try(local.data.ranges, {}) : key => value if !contains(keys(local.map_ranges_internal), key)
  }
...
locals {
  ...
  # External objects
  map_fqdns_external = {
    for key, value in try(local.data.fqdns, {}) : key => value if !contains(keys(local.map_fqdns_internal), key)
  }
...
locals {
  ...
  # Keys of network groups managed internally (from config, not resource outputs - avoids cycle)
  network_groups_internal_keys = toset(concat(
    # Data source network groups
    flatten([for domain in keys(local.data_network_groups) : [for k in keys(local.data_network_groups[domain].items) : "${domain}:${k}"]]),
    # Resource network groups (from domains config, excluding data sources)
    flatten([
      for domain in local.domains : [
        for network_group in try(domain.objects.network_groups, []) : "${domain.name}:${network_group.name}"
  ...
  map_network_groups_external = {
    for key, value in try(local.data.network_groups, {}) : key => value if !contains(local.network_groups_internal_keys, key)
  }
